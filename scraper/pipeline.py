...
- deliverables are first written to a staging folder inside --out, then swapped in together;
- if an old file cannot be replaced (typically books.xlsx open in Excel on Windows), nothing is
  swapped: the previous deliverables stay exactly as they were;
- any failure after the arguments were validated still writes run_log.json with status "failed"
  and the reason, so a scheduler never finds a stale "success" next to a failed run.
"""
...
class OutputLockedError(RuntimeError):
    ...
    def __init__(self, name: str, out_dir: Path) -> None:
        super().__init__(f"{name} is open in another program (close it and run again); "
                         f"the previous files in {out_dir} were left unchanged")
        self.name = name
...
def _swap_in(stage: Path, out: Path, names: list[str]) -> None:
    ...
    for name in names:
        ...
        try:
            ...
            for done in reversed(moved):  # restore the previous set exactly as it was
...
def write_failure_log(settings: RunSettings, log: logging.Logger, exc: BaseException, stage: str,
    ...
    """Record a failed run in run_log.json (best effort: never hides the original error)."""
    ...
    try:
        ...
        event(log, "run_failed", logging.ERROR, stage=stage, error=run_log["error"])
